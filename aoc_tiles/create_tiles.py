...
IMAGE_DIR = SOURCE_CODE_DIR / "aoc_tiles"
...
def get_font(size: int, path: str):
    return ImageFont.truetype(str(SOURCE_CODE_DIR / path), size)
...
def handle_day(day: int, year: int, solutions: list[str], html: HTML, day_scores: DayScores | None, needs_update: bool):
    ...
    day_graphic_path = day_graphic_path.relative_to(SOURCE_CODE_DIR)
...
def handle_year(year: int, day_to_solutions: dict[int, list[str]]):
    ...
    with open(completed_cache_path, "w") as file:
        completed_days = [day for day, scores in leaderboard.items() if scores is not None and scores.time2 is not None]
